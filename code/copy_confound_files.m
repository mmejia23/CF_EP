...
%     dir_data = 'C:\Users\cthul\Dropbox\PROYECTOS\0-PhD\expBA\';
    ...
    for i = 1:size(dir_list,1)
        tsv_file_to_copy = spm_select('FPListRec', dir_sub, 'desc-confounds_timeseries.tsv');
        [~,NAME,EXT] = fileparts(tsv_file_to_copy);
        tsv_file_to_save = fullfile(dir_out, deblank(dir_list(i,:)), 'func', [NAME, EXT]);
        json_file_to_copy = spm_select('FPListRec', dir_sub, 'desc-confounds_timeseries.json');
        [~,NAME,EXT] = fileparts(json_file_to_copy);
        json_file_to_save = fullfile(dir_out, deblank(dir_list(i,:)), 'func', [NAME, EXT]);
